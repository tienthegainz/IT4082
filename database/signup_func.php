<?php
  ...
  if(empty($name) || empty($user) || empty($pass) || empty($age) || empty($weight) || empty($height)){
    header("Location: ../signup.php?signup=empty&name=$name&user=$user&age=$age&weight=$weight&height=$height"); //failed
  }
  else if(!preg_match("/^[a-z A-Z]*$/",$name)){
    header("Location: ../signup.php?signup=char&user=$user&age=$age&weight=$weight&height=$height");
  }
  else if(strlen($user)<5){
    header("Location: ../signup.php?signup=user&name=$name&age=$age&weight=$weight&height=$height"); //failed
  }
  else if($pass != $pass1){
    header("Location: ../signup.php?signup=pass&name=$name&user=$user&age=$age&weight=$weight&height=$height");
  }
  else{
